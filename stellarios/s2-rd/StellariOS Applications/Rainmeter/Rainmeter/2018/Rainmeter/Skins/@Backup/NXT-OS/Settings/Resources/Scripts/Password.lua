...
function SetPass()
	SKIN:Bang('!Update')
	local salt=string.sub(sha256(string.sub(sha256(string.sub(sha256(tostring(math.random(0,2147483647))),math.random(1,32))),math.random(1,40))),16,48)
	local pass1=SKIN:GetVariable('NewPassword1')
	local pass2=SKIN:GetVariable('NewPassword2')
	if pass1==pass2 then
		SKIN:Bang('!WriteKeyValue','Variables','Password2',sha256(pass1..salt),'#@#Passwords.inc')
		SKIN:Bang('!WriteKeyValue','Variables','Salt2',salt,'#@#Passwords.inc')
		SKIN:Bang('[!SetVariable NewPassword1 ""][!SetVariable NewPassword2 ""][!HideMeterGroup SetPass][!HideMeter ErrorText][!Update]')
	else
		SKIN:Bang('[!ShowMeter ErrorText][!Update]')
	end
end
